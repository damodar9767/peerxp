...
const PopupForm2 = ( {closeModal, formdat , type , posttobe , user})  => {
  ...
  useEffect(() => {
    const getPromptDetails = async () => {
        const response = await fetch(`/api/prompt/${posttobe}`)
        const data = await response.json();
              
        const [dat, o] = data.doe.split('T')
      ...
      <div className='modalComponent  pt-4 px-6  bg-white rounded-xl w-[300px] sm:w-[500px]'>
        ...
        <div className='modalmiddle my-6'>
          <form>
            ...
            <input placeholder='Name of the Expanse' className='modal_inputs' type='text'  required  name='name' value={pform.name} onChange={handleChange} />
